...
export function PaymentCardContainer(props: PaymentCardContainerProps) {
	...
	const assetFromLabel =
		variant === "receivable" ? "Request from:" : "Asset from:";
	return (
					...
					<div className="flex xl:flex-row flex-col xl:gap-8 gap-1">
						<div className="flex w-[180px] gap-1">
							...
							<Typography variant="h6">
								{`฿${rentalFee} (฿${
									Math.round((rentalFee / daysDiff + Number.EPSILON) * 100) /
									100
								}/day)`}
							</Typography>
						</div>
...
export function ExpensesCardButton(props: ExpensesCardButtonProps) {
	const { dueDate } = props;
	const dueDateText = `${dueDate.getDate()}/${dueDate.toLocaleString(
		"default",
		{
			month: "2-digit",
			year: "2-digit",
