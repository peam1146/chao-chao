...
export default function Home() {
  ...
  return (
    <main className="container flex w-full bg-background h-full flex-col items-center justify-between my-10">
      <CategoryBlock />
      <div className="flex flex-col w-full gap-20 pt-10">
        {items?.docs?.filter((item) => item?.id !== undefined).at(0) && (
          <PromoteCard
            key={items?.docs?.at(0)?.id}
            id={Number(items?.docs?.at(0)?.id)}
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 2xl:gap-2 lg:gap-3 md:gap-2 gap-3">
          {items?.docs
