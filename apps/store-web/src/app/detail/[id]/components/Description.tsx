...
export default function Description({
	isSelf,
	item,
}: { isSelf: boolean; item: Maybe<Item> }) {
			...
			<div className="flex flex-wrap gap-x-2 gap-y-3">
				{item?.tags?.map((item, index) => (
					<Badge
						variant="outline"
						key={index}
						className="px-3 py-1 border-muted-foreground"
					>
					...
					>
						...
						<div className="h-7 flex justify-between">
							...
							{item?.rentingStatus == Item_rentingStatus.available ? (
								<Typography
									variant="h6"
									className="px-3 py-1 border rounded-full bg-available"
								>
									Available
								</Typography>
							) : (
								<Typography
									variant="h6"
									className="px-3 py-1 border rounded-full bg-unavailable"
								>
									Unavailable
								</Typography>
							)}
