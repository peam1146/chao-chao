    ...
    >
      <div className="flex flex-row items-center justify-center gap-2 max-lg:flex-col">
        <div className="flex flex-row gap-2">
          {uploadStatus === 'default' && <CloudArrowUp className="w-5 h-5 text-primary" />}
          <Typography
            variant="h5"
            className={`${uploadStatus === 'error' ? 'text-system-error' : 'text-primary'}`}
          >
            {uploadText}
          </Typography>
        </div>
        ...
        >
          <div className="flex flex-row gap-2">
            ...
            {!(uploadStatus === 'default') && <CloudArrowUp className="w-5 h-5 text-invert" />}
            {uploadStatus === 'default' ? 'Click here to upload file' : 'Upload again'}
          </div>
        </Button>
      </div>
    </div>
