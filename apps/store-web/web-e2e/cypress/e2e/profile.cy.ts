describe('profile', () => {
  beforeEach(() => {
    ...
    //   cy.wait(1000)
    //   cy.reload()
    //   cy.get('button').contains('Manage Asset', { timeout: 12000 }).should('be.visible')
  ...
  //   cy.get('input[name="province"]').clear()
  //   cy.get('textarea[name="bio"]').clear()
  //   cy.get('input[type = "file"]').clear({ force: true })
  // })
  ...
  //   cy.get('button[type="submit"]').contains('Save').click()
  //   expect(cy.get('p').contains('Short bio must not exceed 10 characters.')).to.exist
  // })
  // it('TC2-6', () => {
  //   cy.get('input[name="firstName"]').type('Fahphon')
  //   cy.get('input[name="lastName"]').type('Jaidee')
